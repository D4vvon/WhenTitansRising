...
{
	if (!FMath::IsNearlyZero(Value, 1e-6f) && GetCharacterMovement()->IsMovingOnGround() || GetCharacterMovement()->IsFalling()) // || GetCharacterMovement()->IsFalling()
	{
...
{
	if (!FMath::IsNearlyZero(Value, 1e-6f) && GetCharacterMovement()->IsMovingOnGround() || GetCharacterMovement()->IsFalling())
	{
		FRotator YawRotator(0.0f, GetControlRotation().Yaw, 0.0f);
...
void AWTR_MainCharacter::SwimForward(float Value)
{
	if (!FMath::IsNearlyZero(Value, 1e-6f) && GetCharacterMovement()->IsSwimming())
	{
...
void AWTR_MainCharacter::SwimRight(float Value)
{
	if (!FMath::IsNearlyZero(Value, 1e-6f) && GetCharacterMovement()->IsSwimming())
	{
		FRotator YawRotator(0.0f, GetControlRotation().Yaw, 0.0f);
...
void AWTR_MainCharacter::SwimUp(float Value)
{
	if (!FMath::IsNearlyZero(Value, 1e-6f) && GetCharacterMovement()->IsSwimming())
	{
